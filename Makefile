# This Makefile is for the Google::Auth extension to perl.
#
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Google/Auth.pm \
	lib/Google/Auth/EnvironmentVars.pm
...
DISTNAME = Google-Auth
DISTVNAME = Google-Auth-0.01
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Google/Auth.pm $(INST_MAN3DIR)/Google::Auth.$(MAN3EXT) \
	  lib/Google/Auth/EnvironmentVars.pm $(INST_MAN3DIR)/Google::Auth::EnvironmentVars.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Implements application default credentials and project ID detection.'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''C.J. Collier <CJCOLLIER@CPAN.ORG>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Google-Auth' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Implements application default credentials and project ID detection.",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "C.J. Collier <CJCOLLIER@CPAN.ORG>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Google-Auth",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Google-Auth" VERSION="0.01">' > Google-Auth.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Implements application default credentials and project ID detection.</ABSTRACT>' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>C.J. Collier &lt;CJCOLLIER@CPAN.ORG&gt;</AUTHOR>' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.28" />' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Google-Auth.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Google-Auth.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/Google/Auth.pm' 'blib/lib/Google/Auth.pm' \
	  'lib/Google/Auth/EnvironmentVars.pm' 'blib/lib/Google/Auth/EnvironmentVars.pm' 
